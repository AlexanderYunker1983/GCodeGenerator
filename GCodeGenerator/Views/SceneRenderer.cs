...
{
    /// <summary>
    /// Renders a pure <see cref="TrajectoryScene"/> (Core data) to a WPF
    /// <see cref="Model3DGroup"/> (plan item 6.2). All 3D geometry helpers
    /// were moved here from <c>PreviewViewModel</c> verbatim (Point3D-based);
    /// the WPF boundary is the <see cref="ToRenderSegments"/> conversion.
    /// </summary>
    public static class SceneRenderer
    {
        ...
        /// <summary>
        /// Point3D-based mirror of a scene segment (WPF boundary).
        /// </summary>
        ...
        public static Model3DGroup Render(TrajectoryScene scene)
        {
            var segments = ToRenderSegments(scene);
            return BuildModelFromSegments(segments);
        }
        ...
        private static List<RenderSegment> ToRenderSegments(TrajectoryScene scene)
        {
            ...
            {
                var render = new RenderSegment
                {
                    Start = ToPoint3D(seg.Start),
                    End = ToPoint3D(seg.End),
                    MoveType = seg.MoveType
                };

                if (seg.InterpolatedPoints != null)
                {
                    render.InterpolatedPoints = new List<Point3D>(seg.InterpolatedPoints.Count);
                    foreach (var point in seg.InterpolatedPoints)
                        render.InterpolatedPoints.Add(ToPoint3D(point));
                }
        ...
        private static Model3DGroup BuildModelFromSegments(List<RenderSegment> segments)
        {
            ...
            {
                var minX = allPointsForBounds.Min(p => p.X);
                var maxX = allPointsForBounds.Max(p => p.X);
                var minY = allPointsForBounds.Min(p => p.Y);
                var maxY = allPointsForBounds.Max(p => p.Y);
                var minZ = allPointsForBounds.Min(p => p.Z);
                var maxZ = allPointsForBounds.Max(p => p.Z);

                var sizeX = maxX - minX;
                var sizeY = maxY - minY;
                var sizeZ = maxZ - minZ;
                var boundsMax = Math.Max(Math.Max(sizeX, sizeY), Math.Max(sizeZ, 1.0));
                var baseThickness = boundsMax * 0.008;
            ...
            if (allPointsForBounds.Count > 0)
            {
                var sizeX = allPointsForBounds.Max(p => p.X) - allPointsForBounds.Min(p => p.X);
                var sizeY = allPointsForBounds.Max(p => p.Y) - allPointsForBounds.Min(p => p.Y);
                var sizeZ = allPointsForBounds.Max(p => p.Z) - allPointsForBounds.Min(p => p.Z);
        ...
        private static void AddCoordinateAxes(Model3DGroup modelGroup, List<RenderSegment> segments, double lineThickness)
        {
            ...
            AddAxisLine(modelGroup, origin, xArrowStart, lineThickness, xMaterial);
            ...
            AddAxisLine(modelGroup, origin, yArrowStart, lineThickness, yMaterial);
            ...
            AddAxisLine(modelGroup, origin, zArrowStart, lineThickness, zMaterial);
        ...
        private static void AddAxisLine(Model3DGroup modelGroup, Point3D start, Point3D end, double thickness, Material material)
        {
        ...
        private static void AddArrowHead(Model3DGroup modelGroup, Point3D baseCenter, Point3D tip, double radius, Material material)
        {
        ...
        private static void AddAxisLabel(Model3DGroup modelGroup, Point3D position, string label, Material material, double size)
        {
        ...
        private static void CreateXShape(MeshGeometry3D geometry, Point3D center, double size)
        {
        ...
        private static void CreateYShape(MeshGeometry3D geometry, Point3D center, double size)
        {
            var thickness = size * 0.15; // Thinner lines for labels
            var halfSize = size * 0.5;
        ...
        private static void CreateZShape(MeshGeometry3D geometry, Point3D center, double size)
        {
            var thickness = size * 0.15; // Thinner lines for labels
            var halfSize = size * 0.5;
        ...
        private static void MergeGeometry(MeshGeometry3D target, MeshGeometry3D source)
        {
        ...
        private static void CreateConeGeometry(MeshGeometry3D geometry, Point3D baseCenter, Point3D tip, double radius)
        {
        ...
        private static void CreateLineGeometry(MeshGeometry3D geometry, Point3D start, Point3D end, double thickness)
        {
        ...
        private static Vector3DCollection CalculateNormals(List<Point3D> positions, List<int> indices)
        {
        ...
        /// </summary>
        private static void AddLineGeometry(MeshGeometry3D targetGeometry, Point3D start, Point3D end, double thickness)
        {
        ...
        /// </summary>
        private static void AddDashedLineGeometry(MeshGeometry3D targetGeometry, Point3D start, Point3D end,
        ...
        private static void AddPointMarkers(Model3DGroup modelGroup, List<RenderSegment> segments, double markerSize)
        {
        ...
        private static void CreateSphereGeometry(MeshGeometry3D geometry, Point3D center, double radius)
        {
            var positions = new List<Point3D>();
            var indices = new List<int>();
            var normals = new List<Vector3D>();

...
namespace GCodeGenerator.Models
{
    ...
    ///
    /// У подач есть и верхний защитный предел. Он не заменяет паспорт станка:
    /// значение ниже общего потолка может быть недопустимо для конкретного
    /// оборудования и проверяется оператором по его документации.
    /// </summary>
    public static class OperationValidation
    {
        ...
        ///
        /// Общий потолок защищает от заведомо абсурдных значений, но не может
        /// угадать паспорт конкретного станка или намерение пользователя:
        /// 3000 мм/мин бывает и ошибкой вместо 300, и легальным режимом.
        /// </summary>

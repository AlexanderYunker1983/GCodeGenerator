...
1. ~~Настройки работы шпинделя (5.3.1)~~
2. ~~Настройки работы СОЖ (5.3.2)~~
3. Настройка системы координат в начале программы (5.3.3)
4. Обработка карманов с уклонными стенками (3.3.1)
5. Импорт из DXF (5.2.1)
6. Ручной и автоматический ремонт контуров из DXF (5.2.2)
7. Фрезеровка под крепеж (2.1.1)
8. Фрезеровка с зенковкой (2.1.2)
9. Фрезеровка с рассверловкой (2.1.3)
10. Черновая и чистовая обработка кармана (3.1.3)
11. ~~Сохранение/чтение проекта операций (5.1.3)~~
...
7. Оптимизация кода преобразованием координат (5.1.4)
...
### 3.3. Специальные операции (Высокий приоритет)

#### 3.3.1. Обработка карманов с уклонными стенками
- **Описание**: Фрезеровка карманов с наклонными стенками (конусные карманы)
- **Параметры**: 
  - Угол наклона стенок
  - Глубина кармана
  - Стратегия обработки (адаптация к уклону)
  - Компенсация инструмента с учетом угла
- **Применение**: Карманы с уклоном для литейных форм, пресс-форм
- **Зависимости**: Нет
...
#### 5.1.3. ~~Сохранение/чтение проекта операций (Высокий приоритет)~~
- ~~**Описание**: Сохранение и загрузка полного проекта со всеми операциями и настройками~~
- ~~**Функции**:~~
  - ~~Сохранение всех операций в файл проекта~~
  - ~~Загрузка проекта из файла~~
  - ~~Сохранение настроек станка и инструментов~~
  - ~~Сохранение параметров операций~~
  - ~~Версионирование формата проекта~~
  - ~~Автосохранение и восстановление после сбоя~~
- ~~**Формат**: Собственный формат проекта (JSON или XML)~~
- ~~**Применение**: Сохранение работы, обмен проектами между пользователями~~
- ~~**Зависимости**: Нет~~

#### 5.1.4. Оптимизация кода преобразованием координат (Средний приоритет)
...
#### 5.2.1. Импорт из DXF (Высокий приоритет)
- **Описание**: Импорт контуров из DXF файлов
- **Функции**:
  - Импорт линий, дуг, полилиний
  - Поддержка различных слоев
  - Масштабирование и трансформация
- **Зависимости**: Библиотека для работы с DXF

#### 5.2.2. Ручной и автоматический ремонт контуров из DXF (Высокий приоритет)
...
#### 5.2.3. Импорт из SVG (Средний приоритет)
- **Описание**: Импорт контуров из SVG файлов
- **Зависимости**: Библиотека для работы с SVG

#### 5.2.4. Экспорт операций (Средний приоритет)
...
#### 5.3.1. ~~Настройки работы шпинделя~~
- ~~**Описание**: Управление шпинделем в операциях обработки~~
- ~~**Параметры**:~~
  - ~~Скорость вращения шпинделя (S-код)~~
  - ~~Включение шпинделя перед операцией (M3/M4)~~
  - ~~Выключение шпинделя в конце операции (M5)~~
  - ~~Направление вращения (по часовой/против часовой)~~
  - ~~Задержка после включения шпинделя~~
  - ~~Настройки по умолчанию в установках станка~~
- ~~**Применение**: Правильное управление шпинделем для всех операций~~
- ~~**Зависимости**: Нет~~

#### 5.3.2. ~~Настройки работы СОЖ~~
- ~~**Описание**: Управление системой охлаждения и смазки~~
- ~~**Параметры**:~~
  - ~~Включение СОЖ перед операцией (M8)~~
  - ~~Выключение СОЖ после операции (M9)~~
  - ~~Режимы работы СОЖ (постоянная, импульсная)~~
  - ~~Настройки по умолчанию в установках станка~~
- ~~**Применение**: Правильное управление СОЖ для продления срока службы инструмента~~
- ~~**Зависимости**: Нет~~

#### 5.3.3. Настройка системы координат в начале программы (Высокий приоритет)

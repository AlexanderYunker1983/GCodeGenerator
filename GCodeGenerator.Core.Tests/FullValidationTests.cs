...
namespace GCodeGenerator.Tests
{
    ...
    public class FullValidationTests
    {
        ...
        public void UndefinedEnumValue_IsRejected()
        {
            ...
            operation = ValidPocket();
            operation.ProcessingDirection = (PocketProcessingDirection)99;
            issues = Check(operation);
            Assert.IsTrue(issues.Any(i => i.Property == nameof(operation.ProcessingDirection)
                                          && i.Code == ValidationCode.NotAllowed),
                "Неизвестное направление обработки не должно менять порядок проходов");
        }

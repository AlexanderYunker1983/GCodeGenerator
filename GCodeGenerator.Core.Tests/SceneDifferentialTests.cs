...
namespace GCodeGenerator.Tests
{
    ...
    /// точки интерполяции.
    ///
    /// Структурный построитель применяет ту же точность Decimals, которую
    /// форматтер записывает в текст: обе стороны видят координаты станка.
    /// Допуск 1e-3 мм оставлен для независимых формул интерполяции дуг.
    ///
    ...
    ///
    /// Выравнивание терпит короткие сегменты старого парсера ради его
    /// исторических особенностей на границе допуска.
    /// </summary>
    [TestClass]
